...
import { inventoryViewConfig } from '../../utils/display_configuration';
...
const EditInventoryForm = () => {
  ...
  useEffect(() => {
    const fetchInventoryById = async () => {
      const inventoryResponse = await makeApiCall(
        `${BASE_URL}${PATH_INVENTORY}(${id})`
      );
      const jsonResp = await inventoryResponse.json();
  ...
  const submitForm = async () => {
    ...
    const resp = await makeApiCall(
      `${BASE_URL}${PATH_INVENTORY}(${formData.InventoryId})`,
      'PATCH',
            JSON.stringify({
        ...otherData,
            CurrentStockUnits: parseInt(CurrentStockUnits),
      })
    );
    if (resp.ok) {
      snackbar.enqueueSnackbar('Successfully updated Inventory', {
        variant: 'success',
      });
      navigate({ pathname: '/inventories' });
    } else {
      ...
      <Grid>
          ...
          <Box display="flex" justifyContent="space-between">
            ...
            <div className="action-buttons">
              ...
              <Button
                ...
                variant="contained"
                color="secondary"
                onClick={() => navigate({ pathname: '/inventories' })}
              >
                Cancel
              </Button>
            </div>
        ...
        <Box marginTop={2} className="form-container">
          <Grid container item lg={12} xs={12}>
            {Object.keys(inventoryViewConfig)?.map((config, ind) => (
              <>
                <Grid item lg={5} md={5} xs={12}>
                  <Box marginTop={1}>
                    ...
                    <Table size="small" className={styles.table}>
                      <TableBody>
                        {inventoryViewConfig[config]?.map(
